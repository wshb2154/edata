 ...
 */
function buildVideoPlug(domid,screenNum){
	// 检查插件是否已经安装过
 ...
 * @email wshb2154@163.com
 */
function haikang(ip,port,username,password){
	this.ip = ip;
	this.port = port;
	 ...
	 * 监控头
	 * @public
	 * @deprecated 推荐使用insetVideo方法
	 */
	this.insetTo = function(videoPlugNum){
		this.type = 1;
		this.plugNum = videoPlugNum;
		this.insetHaikang();
	}
	
	/**
	 * 嵌入到指定的分屏控件子屏中。 
	 * 适用单个监控头或者监控录像机有且只有一个监控头。
	 * @private
	 * @param videoPlugNum 从0开始；
	 *            子屏序号
	 */
	this.insetVideo = function(videoPlugNum){
		this.insetTo(videoPlugNum);
	}
	 ...
	 * @public
	 */
	this.insetVideoDisk = function(){
		this.type = 2;
		this.videosNumInDisk = 0; //监控录像机中已绑定的监控头个数
		this.insetHaikang();
	}
	
	this.insetHaikang = function(){
		var iRet = this.loginAndPlay();
		if (-1 == iRet) {
			showOPInfo(ip + " 准备重新登录！");
	 ...
	 * @private
	 */
	this.loginAndPlay = function(){
		var me = this;
		var iRet = WebVideoCtrl.I_Login(this.ip, 1, this.port, this.username, this.password, {
			success: function (xmlDoc) {
				showOPInfo(me.ip + " 登录成功！");
				setTimeout(function () {				
					if(me.type==1){ //单个监控头
						me.doStartRealPlay(me.plugNum);
					}else if(me.type==2){  //监控录像机
						me.getChannelInfo();
						for(var i=0;i<me.videosNumInDisk;i++){
							me.doStartRealPlay(i);
						}
					}
				}, 10);
			},
			error: function () {
				showOPInfo(me.ip + " 登录失败！");
			}
		});
		if (-1 == iRet) {
			showOPInfo(me.ip + " 已登录过！");
		}
		return iRet;
	};
	 ...
	 * @private
	 */
	this.doStartRealPlay = function(videoPlugNum){
		var oWndInfo = WebVideoCtrl.I_GetWindowStatus(videoPlugNum),
		szIP = this.ip,
		iStreamType = 1,
		iChannelID = (this.type == 1)?1:(videoPlugNum+1),
		bZeroChannel = false,
		szInfo = "";
		...
		if (oWndInfo != null) {// 已经在播放了，先停止
			showOPInfo(szIP + "已有视频在"+videoPlugNum+"号位播放。 " );
			WebVideoCtrl.I_Stop();
			showOPInfo(szIP + "关闭"+videoPlugNum+"号位播放。 " );
		}
		
		startRealPlayLoop();
		//startRealPlayRecursion();
		function startRealPlayLoop(){ //如果失败的话，更换码流类型重新预览。最多尝试4种码流类型
			
			for(var i=0;i<4;i++){
				var iRet = startRealPlay();
				if(0 == iRet){
					break;
				}
				iStreamType++;
		...
		function startRealPlayRecursion(){ //失败的话递归
			if(iStreamType>4) return;
			var iRet = startRealPlay();
			if(0 == iRet) return;
			iStreamType++;
			startRealPlayRecursion();
		...
		function startRealPlay(){
			var iRet = WebVideoCtrl.I_StartRealPlay(szIP, {
				iWndIndex : videoPlugNum,
				iStreamType: iStreamType,
				iChannelID: iChannelID,
				bZeroChannel: bZeroChannel
			});
			if(0 == iRet){
				szInfo = "用"+iStreamType+"码流类型在"+videoPlugNum+"号位开始预览成功！";
			}else{
				szInfo = "用"+iStreamType+"码流类型在"+videoPlugNum+"号位开始预览失败！";
			}
	...
	this.getChannelInfo =function(){
		...
		WebVideoCtrl.I_GetAnalogChannelInfo(szIP, {
			...
			success: function (xmlDoc) {
				...
				$.each(oChannels, function (i) {
					...
					// oSel.append("<option value='" + id + "' bZero='false'>" +
					// name + "</option>");
				});
		...
		WebVideoCtrl.I_GetDigitalChannelInfo(szIP, {
			...
			success: function (xmlDoc) {
				...
				me.videosNumInDisk = oChannels.length;
				$.each(oChannels, function (i) {
					var id = parseInt($(this).find("id").eq(0).text(), 10),
						name = $(this).find("name").eq(0).text(),
			...
			success: function (xmlDoc) {
				...
				$.each(oChannels, function (i) {
					...
					if ("true" == $(this).find("enabled").eq(0).text()) {// 过滤禁用的零通道
						// oSel.append("<option value='" + id + "' bZero='true'>" +
						// name + "</option>");
					}
				});
